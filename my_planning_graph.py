...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Initialize new a_level
        self.a_levels.append(set())
        for action in self.all_actions:
            cand_a_node = PgNode_a(action)
            cand_parents = []
            precond_total = len(cand_a_node.prenodes)
            precond_count = 0

            for prenode in cand_a_node.prenodes:
                for s_node in self.s_levels[level]:
                    if prenode == s_node:
                        cand_parents.append(s_node)
                        precond_count+=1
                # found an action with all prerequisites satisfied
                if precond_count == precond_total:
                    for parent in cand_parents:
                        parent.children.add(cand_a_node)
                        cand_a_node.parents.add(parent)

                    self.a_levels[level].add(cand_a_node)
